...
extension ProgressPhoto {
    ...
    public func isDuplicate(of other: ProgressPhoto) -> Bool {
        return self.assetIdentifier == other.assetIdentifier && self.angle == other.angle
    }
    
    /// Static helper for duplicate detection
    public static func isDuplicate(candidate: (assetIdentifier: String, angle: PhotoAngle), existing: ProgressPhoto) -> Bool {
